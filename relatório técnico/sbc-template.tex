...
\begin{abstract}
  AgroSys is a web-based agricultural management system developed to assist small farmers in organizing crop production. The system allows users to manage plantations, harvests, inventories, sales, and agricultural tasks through a REST API and a modern web interface. The project aims to improve production planning, reduce information loss, and facilitate decision-making by centralizing agricultural data.
\end{abstract}
...
\noindent \textbf{Keywords:} agricultural management, REST API, web system, small farming, information centralization.

\noindent \textbf{Palavras-chave:} gestão agrícola, API REST, sistema web, agricultura familiar, centralização de informações.
...
O \textbf{AgroSys} foi desenvolvido com o objetivo de auxiliar pequenos produtores rurais no gerenciamento de suas atividades agrícolas. O sistema reúne funcionalidades para controle de plantações, estoque, vendas e tarefas, oferecendo uma solução web organizada e de fácil utilização. A utilização de tecnologias modernas, como Node.js, Express, PostgreSQL, React e Next.js, possibilitou o desenvolvimento de uma aplicação segura, escalável e adequada às necessidades propostas.
